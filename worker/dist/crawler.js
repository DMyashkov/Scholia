...
async function crawlSourceWithConversationId(job, source, conversationId) {
    ...
    const discovered = new Set(); // All discovered URLs (including queued)
...
async function crawlPage(url, source, job, conversationId) {
    ...
    try {
        ...
        linkElements.each((_, element) => {
            ...
            try {
                ...
                if (pos >= 0) {
                    if (pos < 50)
                        contextSnippet = rawText.slice(pos, Math.min(rawText.length, pos + CONTEXT_SNIPPET_LENGTH)).trim();
                    else if (pos + anchorText.length > rawText.length - 50)
                        contextSnippet = rawText.slice(Math.max(0, rawText.length - CONTEXT_SNIPPET_LENGTH), rawText.length).trim();
                    else
                        contextSnippet = rawText.slice(Math.max(0, pos - half), Math.min(rawText.length, pos + anchorText.length + half)).trim();
                }
                else {
